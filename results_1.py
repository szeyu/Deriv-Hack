        ...
        if "verification_complete" not in st.session_state:
            with st.spinner("Verifying passport..."):
                ...
                try:
                    ...
                    png_path = os.path.join(temp_dir, "/Users/mackwongyy/Downloads/WhatsApp Image 2024-11-09 at 14.23.31.jpeg")
                    ...
                    if cropped_face is not None:
                        ...
                        cropped_face_path = os.path.join(temp_dir, "cropped_face.png")
                        cv2.imwrite(cropped_face_path, cropped_face)

                        # Store the cropped face file path in session state
                        st.session_state.cropped_face_path = cropped_face_path
                        st.session_state.cropped_face = cropped_face  # If you need the image itself in session state

                        st.write(f"Cropped face saved at: {cropped_face_path}")
                    else:
